...
public class AVLTree<T extends Comparable<T>> {
	...
	private void updateHeight(TreeNode<T> root) {
		...
		else if (root.right == null) root.height = 1+ root.left.height;
		else if (root.left == null)  root.height = 1+ root.right.height;
		else  root.height = 1+ Math.max(root.left.height,root.right.height);
	}
	//
	// getBalance()
